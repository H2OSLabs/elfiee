...
//! ## Architecture
//!
//! Elfiee Core (engine + MCP server) is independent of any specific frontend.
//! Two deployment modes share 100% of engine + MCP server code:
//!
//! ```text
//! elf serve (headless):                  Tauri desktop (embedded Core + GUI):
//! +----------------------------+         +----------------------------------------------+
//! |  Elfiee Core               |         |  Elfiee Core (embedded)                      |
//! |  MCP SSE Server (:47200)   |         |  MCP SSE Server (:47200)                     |
//! |  EngineManager             |         |  EngineManager                               |
//! |  Arc<AppState>             |         |  Arc<AppState> (shared)                       |
//! +----------------------------+         |       |                                       |
//!       ^         ^                      |  React --Tauri IPC--> commands/*.rs           |
//!       |         |                      |  Agent --MCP SSE:47200--> mcp/server.rs       |
//!     Agent     CLI                      +----------------------------------------------+
//! ```
//!
