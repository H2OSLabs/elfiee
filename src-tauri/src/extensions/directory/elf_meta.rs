...
pub const TEMPLATE_FILES: &[TemplateFile] = &[
    ...
    // mcp.json template removed: actual MCP config (.mcp.json) is dynamically
    // generated by perform_enable_io at agent enable time.
    TemplateFile {
...
mod tests {
    ...
    fn test_template_files_count() {
        assert_eq!(TEMPLATE_FILES.len(), 3, "Should have 3 template files");
    }
